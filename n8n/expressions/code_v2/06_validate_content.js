 ...
 * 
 * 사용법:
 * 1. 이 코드를 n8n Code 노드에 복사하세요.
 * 2. 'Parse JSON' 노드 뒤에 연결하여 사용합니다.
 * 3. 각 항목을 검증하며 위반 사항이 발견되면 워크플로우를 중단(에러 발생)합니다.
 */
...
for (const item of $input.all()) {
    ...
    if (!result.valid) {
        ...
        // 항목을 유효하지 않음으로 표시 (다운스트림 참조용)
        item.json._validation = { status: 'error', errors: result.errors };

        // 'If Error' 노드가 감지할 수 있도록 에러 플래그 설정
...
// 위반 사항은 로그로 남기되, 워크플로우를 중단하지 않고 'If Error' 노드로 넘김
if (allViolations.length > 0) {
    ...
    console.log(`❌ Strict Validation Failed for ${allViolations.length} items:\n${errorMsg}. Passing to 'If Error' node.`);
}
