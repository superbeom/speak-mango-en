...
- **필드**:
  ...
  - `memo`: TEXT (사용자 메모/문맥)
  - `created_at`: TIMESTAMPTZ
- **데이터 구조**: 공식 콘텐츠(`expressions`)와 분리하여 데이터 오염을 방지하고 성능을 최적화합니다.

**6. `vocabulary_lists` (단어장 폴더)**
...
- 단어장과 표현(공식/커스텀) 간의 M:N 관계 관리.
- **필드**:
  - `list_id`: UUID (FK)
  - `expression_id`: UUID (FK, 공식 표현, Nullable)
  - `custom_card_id`: UUID (FK, 커스텀 카드, Nullable)
- **제약**: `expression_id`와 `custom_card_id` 중 하나만 필수(Check Constraint).
- **인덱스 전략**:
  - `idx_vocabulary_items_list_created`: 특정 단어장 내 아이템을 최신순으로 정렬하기 위해 `(list_id, created_at desc)` 적용.
  - `idx_vocabulary_items_custom_card_id`: 향후 커스텀 카드 기능 활성화 시 추가 예정 (비어있는 필드가 많으므로 Partial Index 고려).

### 4.3 분석 및 집계 (Analytics & Aggregation)

**8. `ranking_stats` (집계 테이블/View)**

- **목적**: 표현별 인기도 및 학습 통계 집계 (주간 베스트 등)
- **기준**: 사용자가 실제로 학습을 위해 저장한 횟수(**Save count**)를 기준으로 산정.
...
| **플래시카드**        |       불가       | 기본 (앞/뒤 뒤집기) |  SRS + 커스텀 카드   | 단계별 학습 도구                        |
| **심화 학습 뷰**      |        X         |          X          | O (리스닝/퀴즈/강화) | 리스닝 모드, 몰입형 퀴즈 등             |
| **커스텀 단어장**     |        X         |    O (최대 5개)     |      O (무제한)      | 나만의 표현 분류 및 관리                |
| **커스텀 단어 등록**  |        X         |          X          |          O           | 직접 표현 추가 기능                     |
...
### 5.9 마이 페이지 학습 모드 (Advanced Learning Views)

'저장한 표현'과 '학습한 표현'을 기반으로 4가지 전문적인 학습 환경을 제공합니다.

1.  **플래시카드 (Flashcards)**: 앞면(영어) <-> 뒷면(한글) 뒤집기 기반 암기 도구.
    - **개별/전체 뒤집기**: 카드별 또는 화면 내 모든 카드 일괄 전환.
    - **모드 설정**: 표현만 보고 맞추기 / 뜻만 보고 맞추기.
2.  **리스닝 모드 (Listening Mode)**: `dialogue` 데이터 기반의 청취 집중 모드.
    - **Step-by-Step**: 대화문을 문장 단위로 청취하며 스스로 진도 조절.
    - **스크립트 가리기**: 블러 처리로 리스닝 역량 극대화. (커스텀 단어 제외)
3.  **퀴즈 모드 (Quiz Mode)**: 내가 모은 표현들로 생성하는 맞춤형 테스트.
    - **기본형**: 기존 퀴즈 형식 유지.
    - **심화형**: 문장 완성형(Chips), 받아쓰기(Dictation), 롤플레잉(Role Play) 등 몰입형 인터랙션. (커스텀 단어 제외)
4.  **강화 학습 (Reinforcement)**: "오답 노트" 성격의 집중 케어 모드.
    - **Marking**: 학습 도중 📌 Pin 또는 ⚡ Weak 버튼으로 개별 마킹한 표현 관리.

### 5.10 단어장 시스템 (Vocabulary List System)

단순 저장을 넘어 사용자가 직접 큐레이션할 수 있는 폴더 시스템입니다.

- **폴더 관리**: 커스텀 폴더(예: 여행 영어) 생성 및 대량 편집(Bulk Move/Copy/Delete).
...
Pro 사용자가 외부 표현을 직접 등록하여 Speak Mango의 학습 도구(플래시카드, 강화학습)를 활용할 수 있게 합니다.

- **데이터 호환성**: 텍스트 기반 기능(플래시카드, 강화학습)은 지원하나, 데이터가 없는 기능(리스닝, 퀴즈)은 대상에서 자동 제외됩니다.
...
- **커스텀 카드**:
  - [ ] Pro 전용 커스텀 표현 등록 기능 및 `user_custom_cards` 연동.

### Phase 7: 폴리싱 및 분석

- **디자인 고도화**:
  - [ ] 주간 베스트 표현 리바인딩 및 골드/그린 테마 적용.
- **최종 검증**:
  - [ ] 무료 -> 유료 전환 시 데이터 동기화 시나리오 점검.
