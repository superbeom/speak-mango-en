...
- [1. Mobile Optimization & Interaction (모바일 최적화 및 인터랙션)](#1-mobile-optimization-interaction-모바일-최적화-및-인터랙션)
  - [1.1 Mobile Detection Strategy (모바일 감지)](#11-mobile-detection-strategy-모바일-감지)
  - [1.2 Conditional Hover Effects (조건부 호버 효과)](#12-conditional-hover-effects-조건부-호버-효과)
  - [1.3 Scroll To Top Interaction (상단 이동 시스템)](#13-scroll-to-top-interaction-상단-이동-시스템)
  - [1.4 Responsive UI Architecture (반응형 UI 아키텍처)](#14-responsive-ui-architecture-반응형-ui-아키텍처)
- [2. UI Automation Logic (UI 자동화)](#2-ui-automation-logic-ui-자동화)
  - [2.1 Auto-Scroll to Active Filter (필터 자동 스크롤)](#21-auto-scroll-to-active-filter-필터-자동-스크롤)
- [3. Animation Logic (애니메이션 로직)](#3-animation-logic-애니메이션-로직)
  - [3.1 Bidirectional Infinite Scroll (양방향 무한 스크롤)](#31-bidirectional-infinite-scroll-양방향-무한-스크롤)
  - [3.2 Drag Acceleration (드래그 가속)](#32-drag-acceleration-드래그-가속)
  - [3.3 Manual Animation & Event Delegation (수동 애니메이션 및 이벤트 위임)](#33-manual-animation-event-delegation-수동-애니메이션-및-이벤트-위임)
- [4. Internationalization (i18n) Engine](#4-internationalization-i18n-engine)
  - [4.1 Multi-Language Expansion (Proxy & Detection)](#41-multi-language-expansion-proxy-detection)
  - [4.2 Server-Side Dictionary Loading](#42-server-side-dictionary-loading)
  - [4.3 Client-Side Data Injection (Context API)](#43-client-side-data-injection-context-api)
  - [4.4 International SEO Implementation (국제 SEO 구현)](#44-international-seo-implementation-국제-seo-구현)
- [5. Data Architecture & Optimization](#5-data-architecture-optimization)
  - [5.1 ISR (Incremental Static Regeneration)](#51-isr-incremental-static-regeneration)
  - [5.2 Supabase Client Separation](#52-supabase-client-separation)
    - [5.2.1 Secure Authentication Bridge (Custom JWT)](#521-secure-authentication-bridge-custom-jwt)
    - [5.2.2 Full RLS Enforcement (전면 보안 강화)](#522-full-rls-enforcement-전면-보안-강화)
  - [5.3 Client-Side State Management (Zustand)](#53-client-side-state-management-zustand)
    - [5.3.1 SWR Optimistic Updates for User Actions](#531-swr-optimistic-updates-for-user-actions)
    - [5.3.2 Parallel Async Operation Optimization](#532-parallel-async-operation-optimization)
  - [5.4 Vocabulary List Optimization & Request Deduplication](#54-vocabulary-list-optimization-request-deduplication)
    - [5.4.1 Global Mutation for Default Vocabulary List Sync](#541-global-mutation-for-default-vocabulary-list-sync)
- [6. Search & Navigation Logic](#6-search-navigation-logic)
  - [6.1 URL Query Parameter State](#61-url-query-parameter-state)
  - [6.2 Centralized Routing System (라우트 중앙 관리)](#62-centralized-routing-system-라우트-중앙-관리)
  - [6.3 Smart Tag Detection (스마트 태그 감지)](#63-smart-tag-detection-스마트-태그-감지)
  - [6.4 Sticky UI Interaction (스티키 UI 상호작용)](#64-sticky-ui-interaction-스티키-ui-상호작용)
  - [6.5 Locale-Specific Search (로케일별 검색)](#65-locale-specific-search-로케일별-검색)
  - [6.6 Duplicate Search Prevention (중복 검색 방지)](#66-duplicate-search-prevention-중복-검색-방지)
  - [6.7 Database Indexing for Search (검색 인덱스 최적화)](#67-database-indexing-for-search-검색-인덱스-최적화)
  - [6.8 Auth Navigation Loop Handling (인증 네비게이션 루프 처리)](#68-auth-navigation-loop-handling-인증-네비게이션-루프-처리)
  - [6.9 Random Feed Optimization (랜덤 피드 최적화)](#69-random-feed-optimization-랜덤-피드-최적화)
- [7. My Page & User Profile Implementation (마이페이지 및 사용자 프로필)](#7-my-page-user-profile-implementation-마이페이지-및-사용자-프로필)
  - [7.1 Remote Image Handling (`next.config.ts`)](#71-remote-image-handling-nextconfigts)
  - [7.2 Dashboard Architecture (`/me`)](#72-dashboard-architecture-/me)
  - [7.3 Vocabulary Toolbar Optimization (툴바 최적화)](#73-vocabulary-toolbar-optimization-툴바-최적화)
  - [7.4 Vocabulary Bulk Operations (일괄 이동 및 복사)](#74-vocabulary-bulk-operations-일괄-이동-및-복사)
  - [7.5 Vocabulary Plan Status (플랜 사용 현황)](#75-vocabulary-plan-status-플랜-사용-현황)
- [8. Audio Playback System (오디오 재생 시스템)](#8-audio-playback-system-오디오-재생-시스템)
  - [8.1 Web Audio API & Volume Amplification (볼륨 증폭)](#81-web-audio-api-volume-amplification-볼륨-증폭)
  - [8.1.1 In-App Browser & iOS Compatibility (인앱 브라우저 호환성)](#811-in-app-browser-ios-compatibility-인앱-브라우저-호환성)
  - [8.2 Path Resolution & Storage Strategy (경로 해석 및 저장 전략)](#82-path-resolution-storage-strategy-경로-해석-및-저장-전략)
  - [8.3 Global Audio Synchronization (전역 동기화)](#83-global-audio-synchronization-전역-동기화)
  - [8.4 Feature Gating Infrastructure (권한 제어)](#84-feature-gating-infrastructure-권한-제어)
  - [8.5 Context-Aware Icon Logic (상태별 아이콘 로직)](#85-context-aware-icon-logic-상태별-아이콘-로직)
  - [8.6 Sequential Playback ("Play All" 멀티 재생)](#86-sequential-playback-play-all-멀티-재생)
  - [8.7 Audio Loading Stabilization (로딩 안정화)](#87-audio-loading-stabilization-로딩-안정화)
  - [8.8 Media Session API Integration (잠금 화면 제어)](#88-media-session-api-integration-잠금-화면-제어)
  - [8.9 Lazy Initialization Strategy (지연 초기화 전략)](#89-lazy-initialization-strategy-지연-초기화-전략)
  - [8.10 Singleton Architecture for Sequential Playback (싱글턴 아키텍처)](#810-singleton-architecture-for-sequential-playback-싱글턴-아키텍처)
  - [8.11 Stable Event Handler Pattern (안정적 핸들러 패턴)](#811-stable-event-handler-pattern-안정적-핸들러-패턴)
- [9. Skeleton Loading Implementation (스켈레톤 구현 상세)](#9-skeleton-loading-implementation-스켈레톤-구현-상세)
- [10. Navigation State Persistence (네비게이션 상태 보존)](#10-navigation-state-persistence-네비게이션-상태-보존)
  - [10.1 Multi-Cache Architecture](#101-multi-cache-architecture)
  - [10.2 Real-time Scroll Tracking](#102-real-time-scroll-tracking)
  - [10.3 Robust Scroll Restoration (Recursive RAF)](#103-robust-scroll-restoration-recursive-raf)
  - [10.4 Detail Page Scroll Reset (상세 페이지 스크롤 리셋 전략)](#104-detail-page-scroll-reset-상세-페이지-스크롤-리셋-전략)
  - [10.5 Performance Optimization (성능 최적화)](#105-performance-optimization-성능-최적화)
  - [10.6 Quiz State Persistence (퀴즈 상태 유지 전략)](#106-quiz-state-persistence-퀴즈-상태-유지-전략)
- [11. Automation Pipeline (n8n & AI)](#11-automation-pipeline-n8n-ai)
  - [11.1 Pre-fetch Duplicate Check](#111-pre-fetch-duplicate-check)
  - [11.2 Strict JSON Parsing](#112-strict-json-parsing)
  - [11.3 Structured Prompt Engineering](#113-structured-prompt-engineering)
  - [11.4 Modular Workflow Management (워크플로우 모듈화)](#114-modular-workflow-management-워크플로우-모듈화)
  - [11.5 Backfill & Multi-Language Merge Strategy (백필 병합 전략)](#115-backfill-multi-language-merge-strategy-백필-병합-전략)
  - [11.6 Batch Processing Optimization (Backfill Efficiency)](#116-batch-processing-optimization-backfill-efficiency)
  - [11.7 Strict Prompt Engineering (No Mixed Language)](#117-strict-prompt-engineering-no-mixed-language)
  - [11.8 Content Verification Logic (Strict Validation)](#118-content-verification-logic-strict-validation)
  - [11.9 Quiz Structure Validation (퀴즈 구조 검증)](#119-quiz-structure-validation-퀴즈-구조-검증)
  - [11.10 Single-Shot AI Generation (V2 Optimization)](#1110-single-shot-ai-generation-v2-optimization)
  - [11.11 Meaning Field Cleanup & Strict Punctuation Validation (데이터 정제 및 엄격한 문장 부호 검증)](#1111-meaning-field-cleanup-strict-punctuation-validation-데이터-정제-및-엄격한-문장-부호-검증)
- [12. Design System & Global Styling (디자인 시스템 및 전역 스타일링)](#12-design-system-global-styling-디자인-시스템-및-전역-스타일링)
  - [12.1 Semantic Theme Variables (시맨틱 테마 변수)](#121-semantic-theme-variables-시맨틱-테마-변수)
  - [12.2 Reusable Utility Classes (공통 유틸리티)](#122-reusable-utility-classes-공통-유틸리티)
  - [12.3 Conditional Styling with `cn()` (조건부 스타일링 유틸리티)](#123-conditional-styling-with-cn-조건부-스타일링-유틸리티)
- [13. Learning Mode System (학습 모드 시스템)](#13-learning-mode-system-학습-모드-시스템)
  - [13.1 View Mode State Machine (뷰 모드 상태 머신)](#131-view-mode-state-machine-뷰-모드-상태-머신)
  - [13.2 State Preservation Logic (상태 보존 로직)](#132-state-preservation-logic-상태-보존-로직)
  - [13.3 English & Translation Interaction](#133-english-translation-interaction)
- [14. Performance Optimization Techniques (성능 최적화 기법)](#14-performance-optimization-techniques-성능-최적화-기법)
  - [14.1 Server-Side Waterfall Removal (서버 측 워터폴 제거)](#141-server-side-waterfall-removal-서버-측-워터폴-제거)
  - [14.2 React Component Rendering Optimization (리렌더링 최적화)](#142-react-component-rendering-optimization-리렌더링-최적화)
  - [14.3 Database Search Optimization (검색 쿼리 최적화)](#143-database-search-optimization-검색-쿼리-최적화)
  - [14.4 Scroll Event Optimization (스크롤 이벤트 최적화)](#144-scroll-event-optimization-스크롤-이벤트-최적화)
  - [14.5 Database Random Sampling (RPC 기반 랜덤 샘플링)](#145-database-random-sampling-rpc-기반-랜덤-샘플링)
  - [14.6 Request Deduplication (Server-Side Caching)](#146-request-deduplication-server-side-caching)
  - [14.7 Data Fetching Strategy (SWR Adoption)](#147-data-fetching-strategy-swr-adoption)
- [15. Shared Header & Unified Navigation (공용 헤더 및 통합 내비게이션)](#15-shared-header-unified-navigation-공용-헤더-및-통합-내비게이션)
  - [15.1 MainHeader: Single Header Strategy](#151-mainheader-single-header-strategy)
  - [15.2 Self-Link Disabling Optimization (자기 참조 링크 최적화)](#152-self-link-disabling-optimization-자기-참조-링크-최적화)
- [16. Vocabulary Management Architecture (보관함 시스템 아키텍처)](#16-vocabulary-management-architecture-보관함-시스템-아키텍처)
  - [16.1 VocabularyView Hook (`useVocabularyView`)](#161-vocabularyview-hook-usevocabularyview)
  - [16.2 Controlled Grid & Sibling Toolbar Pattern](#162-controlled-grid-sibling-toolbar-pattern)
- [17. Service Essentials Implementation (시스템 필수 요소 구현)](#17-service-essentials-implementation-시스템-필수-요소-구현)
  - [17.1 PWA & Splash Screen Strategy (스플래시 스크린 전략)](#171-pwa-splash-screen-strategy-스플래시-스크린-전략)
  - [17.2 Dynamic SEO & Open Graph (동적 SEO)](#172-dynamic-seo-open-graph-동적-seo)
  - [17.3 International SEO Optimization (국제 SEO 최적화)](#173-international-seo-optimization-국제-seo-최적화)
  - [17.4 Dynamic Keyword Localization Strategy (동적 키워드 현지화 전략)](#174-dynamic-keyword-localization-strategy-동적-키워드-현지화-전략)
  - [17.5 PWA Theme Color (동적 테마 컬러)](#175-pwa-theme-color-동적-테마-컬러)
  - [17.6 Type-Safe i18n Architecture (타입 안전 i18n)](#176-type-safe-i18n-architecture-타입-안전-i18n)
  - [17.7 i18n Locale Language Consistency Validation (언어팩 일관성 검증)](#177-i18n-locale-language-consistency-validation-언어팩-일관성-검증)
- [18. Analytics Implementation (사용자 행동 분석)](#18-analytics-implementation-사용자-행동-분석)
  - [18.1 Environment-Based Configuration (환경별 설정)](#181-environment-based-configuration-환경별-설정)
  - [18.2 Type-Safe Event Tracking (타입 안전한 이벤트 추적)](#182-type-safe-event-tracking-타입-안전한-이벤트-추적)
  - [18.3 Automatic Page View Tracking (자동 페이지 뷰 추적)](#183-automatic-page-view-tracking-자동-페이지-뷰-추적)
  - [18.4 Development vs Production Behavior (개발/프로덕션 동작 분리)](#184-development-vs-production-behavior-개발/프로덕션-동작-분리)
  - [18.5 Event Taxonomy (이벤트 분류 체계)](#185-event-taxonomy-이벤트-분류-체계)
  - [18.6 Integration with Next.js App Router (Next.js 통합)](#186-integration-with-nextjs-app-router-nextjs-통합)
  - [18.7 Component-Level Event Tracking (컴포넌트 레벨 이벤트 추적)](#187-component-level-event-tracking-컴포넌트-레벨-이벤트-추적)
  - [18.8 Analytics Module Organization (모듈 구조)](#188-analytics-module-organization-모듈-구조)
- [19. Share Functionality Implementation (공유 기능 구현)](#19-share-functionality-implementation-공유-기능-구현)
  - [19.1 ShareButton Component Architecture (컴포넌트 아키텍처)](#191-sharebutton-component-architecture-컴포넌트-아키텍처)
  - [19.2 Web Share API Integration (네이티브 공유 통합)](#192-web-share-api-integration-네이티브-공유-통합)
  - [19.3 Clipboard Fallback Strategy (클립보드 폴백)](#193-clipboard-fallback-strategy-클립보드-폴백)
  - [19.4 Toast Notification System (토스트 알림 시스템)](#194-toast-notification-system-토스트-알림-시스템)
  - [19.5 Event Propagation Prevention (이벤트 전파 방지)](#195-event-propagation-prevention-이벤트-전파-방지)
  - [19.6 Card Integration with Absolute Positioning (카드 통합)](#196-card-integration-with-absolute-positioning-카드-통합)
  - [19.7 Analytics Integration (분석 통합)](#197-analytics-integration-분석-통합)
  - [19.8 Internationalization (다국어 지원)](#198-internationalization-다국어-지원)
- [20. Marketing Asset Generation System (Marketing Studio)](#20-marketing-asset-generation-system-marketing-studio)
  - [20.1 Client-Side Image Rendering (`html-to-image`)](#201-client-side-image-rendering-html-to-image)
  - [20.2 Automation with Playwright (자동화 스크립트)](#202-automation-with-playwright-자동화-스크립트)
  - [20.3 Localized Asset Generation (다국어 이미지 생성)](#203-localized-asset-generation-다국어-이미지-생성)
  - [20.4 Layout Adaptability for Long Content (유동적 레이아웃)](#204-layout-adaptability-for-long-content-유동적-레이아웃)
  - [20.5 Static Capture Mode (정적 캡처 모드)](#205-static-capture-mode-정적-캡처-모드)
- [21. User System & Authentication (사용자 시스템 및 인증)](#21-user-system-authentication-사용자-시스템-및-인증)
  - [21.1 Refresh Token & Database Session Strategy (세션 관리 전략)](#211-refresh-token-database-session-strategy-세션-관리-전략)
  - [21.2 NextAuth v5 Architecture (인증 아키텍처)](#212-nextauth-v5-architecture-인증-아키텍처)
  - [21.3 Supabase Adapter & Custom Fields (어댑터 및 커스텀 필드)](#213-supabase-adapter-custom-fields-어댑터-및-커스텀-필드)
  - [21.4 Type-Safe Session Access (타입 안전 인증 접근)](#214-type-safe-session-access-타입-안전-인증-접근)
  - [21.5 Schema View Strategy (인증 스키마 전략)](#215-schema-view-strategy-인증-스키마-전략)
- [22. Vocabulary System (단어장 시스템)](#22-vocabulary-system-단어장-시스템)
  - [22.1 Design Philosophy (Implicit Default)](#221-design-philosophy-implicit-default)
  - [22.2 Component Architecture (컴포넌트 구조)](#222-component-architecture-컴포넌트-구조)
  - [22.3 Hybrid Repository Workflow (하이브리드 워크플로우)](#223-hybrid-repository-workflow-하이브리드-워크플로우)
  - [22.4 Long Press Interaction (기본 단어장 변경)](#224-long-press-interaction-기본-단어장-변경)
  - [22.5 Synchronization & Selection Logic](#225-synchronization-selection-logic)
  - [22.6 Database Schema & Triggers](#226-database-schema-triggers)
  - [22.7 Vocabulary Pagination (단어장 페이지네이션)](#227-vocabulary-pagination-단어장-페이지네이션)
- [23. Component Refactoring & Reusability (컴포넌트 리팩토링 및 재사용성)](#23-component-refactoring-reusability-컴포넌트-리팩토링-및-재사용성)
  - [23.1 Unified Action Bar (`ExpressionActions.tsx`)](#231-unified-action-bar-expressionactionstsx)
  - [23.2 Modal Event Isolation (이벤트 격리)](#232-modal-event-isolation-이벤트-격리)
...
- [25. Pro Tier Infrastructure & Advanced Vocabulary Management](#25-pro-tier-infrastructure-advanced-vocabulary-management)
  ...
  - [25.3 Global Confirmation Dialog (`useConfirm`)](#253-global-confirmation-dialog-useconfirm)
  - [25.4 Database Tier Management](#254-database-tier-management)
- [26. UI Component Architecture (Shadcn Style)](#26-ui-component-architecture-shadcn-style)
  - [26.1 Base Component Foundation (`button.tsx`)](#261-base-component-foundation-buttontsx)
...
### 5.4.1 Global Mutation for Default Vocabulary List Sync

단일 리스트의 기본(Default) 설정 변경이 앱 내 다른 리스트들의 '기본 마크'에 미치는 영향을 제어하기 위해 전역 캐시 무효화 전략을 사용합니다.

- **Objective**: 기본 단어장은 단 하나만 존재해야 하며, 설정 변경 시 이전에 기본이었던 다른 리스트들의 UI가 즉시 갱신되어야 함.
- **Implementation**:
  - `useSWRConfig`의 `globalMutate` 함수를 활용합니다.
  - 패턴 매칭 필터를 통해 `vocabulary-details` 세그먼트를 가진 모든 캐시 키를 식별합니다.

- 현재 보고 있는 리스트를 제외한 다른 모든 리스트 상세 데이터의 `revalidate` 옵션을 `true`로 설정하여, 사용자가 해당 페이지로 돌아가면 서버에서 최신 상태를 강제로 다시 불러오도록 유도합니다.
  - **Benefit**: 불필요한 전체 페이지 새로고침 없이도 앱 전역의 데이터 정합성을 유지하며, 프리미엄 티어(Pro) 사용자에게 매끄러운 단어장 전환 경험을 제공합니다.
...
- **Shared Components**: `components/ui/Skeletons.tsx`에서 중앙 관리합니다.
  - **`SkeletonNavbar` (Context-aware)**: `page` Prop(HOME, DETAIL, QUIZ, MY_PAGE)에 따라 헤더 내부 요소(슬로건, 뒤로가기 버튼 등)를 조건부 스켈레톤으로 처리하여 로직 중복을 최소화합니다.
  ...
  - **`SkeletonVocabularyList`**: 모달용 리스트 아이템 스켈레톤입니다.
  - **`SkeletonProfileHeader` & `SkeletonStudyModesGrid`**: 마이페이지 전용 컴포넌트로, 프로필 정보와 2x2 학습 모드 카드 아웃라인을 재현합니다.
  - **`SkeletonVocabularyListSection`**: 마이페이지 하단과 단어장 컨테이너에서 공용으로 쓰이는 섹션 스켈레톤(제목+그리드)입니다.
...
## 15. Shared Header & Unified Navigation (공용 헤더 및 통합 내비게이션)

모든 상세 페이지에서 일관된 사용자 동선과 네비게이션 경험을 제공하기 위한 아키텍처입니다.

### 15.1 MainHeader: Single Header Strategy

- **Unified Logic**: 기존의 페이지별 별도 헤더(`Header` + `BackButton`) 대신 `MainHeader` 하나로 모든 네비게이션 요구사항을 처리합니다.
- **Conditional Brand Area**:
...
사용자별 단어장을 관리하고 대량 편집을 지원하기 위한 고도화된 UI/UX 구조입니다.

### 16.1 VocabularyView Hook (`useVocabularyView`)

- **Role**: 보관함 내의 모든 뷰 상태(선택 모드 여부, 카드 형태(Compact/Full), 선택된 아이템 목록)를 중앙 관리하는 커스텀 훅입니다.
- **Benefits**:
  - **State Isolation**: UI 컴포넌트(`VocabularyItemsGrid`)로부터 비즈니스 로직을 완벽히 분리합니다.
  - **Consistency**: 로컬 보관함과 리모트 보관함(Pro용) 상세 페이지에서 동일한 상태 로직을 공유할 수 있습니다.

### 16.2 Controlled Grid & Sibling Toolbar Pattern

- **Architecture Shift**: `VocabularyItemsGrid` 내부의 툴바 로직을 `VocabularyToolbar`로 독립시키고, 두 컴포넌트를 형제(Sibling) 관계로 재배치했습니다.
- **Infrastructure**:
  - **`VocabularyToolbar`**: 스티키(Sticky) 동작 시 그림자와 배경 조절을 통해 헤더와 자연스럽게 연결되는 플로팅 UI를 담당합니다.
  - **`VocabularyItemsGrid`**: 순수하게 상태(`props`)를 받아 리스트를 렌더링하는 데만 집중하는 Controlled Component로 전환되었습니다.
- **Performance**: 아이템 선택 시 전체 그리드가 아닌 상태를 공유하는 컴포넌트만 효율적으로 업데이트되도록 설계되었습니다.

## 17. Service Essentials Implementation (시스템 필수 요소 구현)

서비스 품질을 결정짓는 3대 요소(PWA, SEO, i18n)에 대한 기술적 구현 상세입니다.

### 17.1 PWA & Splash Screen Strategy (스플래시 스크린 전략)
...
### 17.2 Dynamic SEO & Open Graph (동적 SEO)
...
### 17.3 International SEO Optimization (국제 SEO 최적화)
...
### 17.4 Dynamic Keyword Localization Strategy (동적 키워드 현지화 전략)
...
### 17.6 Type-Safe i18n Architecture (타입 안전 i18n)
...
### 17.7 i18n Locale Language Consistency Validation (언어팩 일관성 검증)
...
## 18. Analytics Implementation (사용자 행동 분석)

Google Analytics 4를 Next.js 16 App Router 환경에 통합하여 사용자 행동 데이터를 수집하고 분석하는 시스템입니다.

### 18.1 Environment-Based Configuration (환경별 설정)
...
### 18.2 Type-Safe Event Tracking (타입 안전한 이벤트 추적)
...
### 18.3 Automatic Page View Tracking (자동 페이지 뷰 추적)
...
### 18.4 Development vs Production Behavior (개발/프로덕션 동작 분리)
...
### 18.6 Integration with Next.js App Router (Next.js 통합)
...
### 18.7 Component-Level Event Tracking (컴포넌트 레벨 이벤트 추적)
...
### 18.8 Analytics Module Organization (모듈 구조)
...
## 19. Share Functionality Implementation (공유 기능 구현)

사용자가 표현을 소셜 미디어나 메신저로 공유할 수 있도록 Web Share API와 Clipboard API를 활용한 크로스 플랫폼 공유 시스템입니다.

### 19.1 ShareButton Component Architecture (컴포넌트 아키텍처)
...
### 19.2 Web Share API Integration (네이티브 공유 통합)
...
### 19.3 Clipboard Fallback Strategy (클립보드 폴백)
...
### 19.4 Toast Notification System (토스트 알림 시스템)
...
### 19.5 Event Propagation Prevention (이벤트 전파 방지)
...
### 19.6 Card Integration with Absolute Positioning (카드 통합)
...
### 19.7 Analytics Integration (분석 통합)
...
### 19.8 Internationalization (다국어 지원)
...
## 20. Marketing Asset Generation System (Marketing Studio)

SNS 마케팅을 위한 고화질 에셋을 생성하고 대량 자동화하기 위한 기술적 구현 상세입니다.

### 20.1 Client-Side Image Rendering (`html-to-image`)
...
### 20.2 Automation with Playwright (자동화 스크립트)
...
### 20.3 Localized Asset Generation (다국어 이미지 생성)
...
### 20.4 Layout Adaptability for Long Content (유동적 레이아웃)
...
### 20.5 Static Capture Mode (정적 캡처 모드)
...
## 21. User System & Authentication (사용자 시스템 및 인증)

NextAuth(Auth.js v5)를 기반으로 구축된 보안 중심의 사용자 인증 및 권한 관리 시스템입니다.

### 21.1 Refresh Token & Database Session Strategy (세션 관리 전략)
...
### 21.2 NextAuth v5 Architecture (인증 아키텍처)
...
### 21.3 Supabase Adapter & Custom Fields (어댑터 및 커스텀 필드)
...
### 21.4 Type-Safe Session Access (타입 안전 인증 접근)
...
### 21.5 Schema View Strategy (인증 스키마 전략)
...
## 22. Vocabulary System (단어장 시스템)

단일 '저장' 버튼을 넘어, 사용자가 직접 분류하고 관리할 수 있는 폴더 구조의 단어장 시스템입니다.

### 22.1 Design Philosophy (Implicit Default)
...
### 22.3 Hybrid Repository Workflow (하이브리드 워크플로우)
...
### 22.4 Long Press Interaction (기본 단어장 변경)
...
### 22.5 Synchronization & Selection Logic
...
### 23.2 Modal Event Isolation (이벤트 격리)
...
## 24. Error Handling Architecture (에러 핸들링 아키텍처)

사용자에게 일관된 에러 경험을 제공하고 디버깅 효율성을 높이기 위해 중앙 집중식 에러 관리 시스템을 구축했습니다.

### 24.1 Centralized Error Hook (`useAppErrorHandler`)
...
### 24.2 Unified Error Types (`types/error.ts`)
