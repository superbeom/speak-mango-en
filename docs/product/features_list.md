...
- **핵심 액션 (Interactions)**:
  - **Save (저장)**: 나만의 단어장에 담아 나중에 다시 공부하기 위한 북마크 기능.
    - **커스텀 단어장 (Vocabulary Lists)**: 'Daily English', 'Travel' 등 사용자가 직접 이름을 붙인 무제한(Pro) 또는 최대 5개(Free)의 리스트를 생성하고 관리할 수 있습니다.
    - **Quick Save & Manage**: 버튼 클릭 시 기본 저장, 롱 프레스(Long Press) 시 저장할 리스트를 선택할 수 있는 모달이 노출됩니다.
    - **Auto-Sync Integrity**: 단어장에서 표현을 모두 제거하면 메인 저장 상태도 자동으로 해제되는 유기적인 상태 동기화를 지원합니다.
