...
- **핵심 액션 (Interactions)**:
  ...
  - **Learn (학습 완료)**: 마스터한 표현을 체크하여 학습 진행도 관리 및 목록 최적화.
    ...
    - **Global Management (Staged Area)**: 단어장 관리 모달을 전역 스토어(`useVocabularyModalStore`)로 통합하여 앱 어디서든 일관된 저장 환경을 제공하며, 비동기 동기화 시 레이스 컨디션 방지 로직을 갖춘 **Staged Area UX**를 완성했습니다.
