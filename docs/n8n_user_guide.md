...
- **Supabase 프로젝트**: 데이터베이스가 구축되어 있어야 합니다.
- **Google Gemini API Key**: AI 콘텐츠 생성을 위한 API 키가 필요합니다.

### 워크플로우 가져오기 (Import)

1.  n8n 대시보드에서 `Import from File`을 선택합니다.
2.  프로젝트 내 `docs/n8n_workflow_template.json` 파일을 업로드합니다.
...
- **백업**: `n8n_data/` 폴더는 `.gitignore`에 포함되어 있으므로, 중요한 워크플로우 변경 사항은 `json` 파일로 내보내어 버전 관리하세요.
