...
1.  **세션 보안 및 데이터 노출 방지**:
    - **비로그인(Anonymous) 게이팅 도입**: 로그아웃 상태에서도 로컬 스토리지에 남은 Free 유저의 Save/Learn 데이터가 UI에 노출되는 버그를 수정했습니다.
    - **`isAuthenticated` 가드 적용**: `useUserActions`의 `hasAction` 및 `isLoading` 계산 시 인증 여부를 먼저 체크하도록 개선하여, 비로그인 상태에서는 항상 `false`를 반환하도록 게이팅했습니다.
    - **데이터 보존 전략**: 로컬 데이터를 강제로 삭제하는 대신 읽기 시점에서 차단함으로써, 사용자가 다시 로그인했을 때의 데이터 연속성은 그대로 유지했습니다.
