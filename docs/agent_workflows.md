...
## 4. 에이전트 스킬 가이드 (Agent Skills)

**가이드 파일**: `docs/agent_skills_guide.md`

### 💡 목적

- Vercel 엔지니어링 팀의 전문적인 React/Next.js 성능 최적화 지침 및 웹 디자인 가이드라인을 에이전트에게 적용합니다.
- 대화 중 에이전트가 "자동으로" 해당 모범 사례를 참고하여 답변하거나 코드를 작성하도록 유도합니다.

### 📋 주요 스킬

1. **Vercel React 베스트 프랙티스**: Waterfall 제거, 번들 최적화, 서버 성능 개선 등 45가지 성능 규칙.
2. **웹 디자인 가이드라인**: 접근성, UX, 디자인 일관성 체크.

### 🚀 활용 예시

- "이 컴포넌트의 성능 문제를 Vercel 가이드라인에 맞게 리뷰해줘."
- "내 UI 코드가 웹 접근성 기준을 잘 따르고 있는지 확인해줘."
